...
if($num>0){
    ...
    $query2 = "SELECT GROUP_CONCAT(parte.nome ORDER BY parte.Id ASC SEPARATOR ',') as partes FROM fase 
    ...
    if($num2>0){
        ...
        echo json_encode(array(
            "message" => "Não foi possível gerar a lista de partes do campeonato."
        ));
    }
    ...
    http_response_code(401);
    echo json_encode(array(
        "message" => "Não foi possível gerar a lista."
    ));
}
